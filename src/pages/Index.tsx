...
const Index = () => {
  ...
  const handleBatchAnalyze = () => {
    ...
    navigate('/report-config', { 
      state: { 
        selectedStocks: selectedStocks.map(symbol => {
          const stock = uploadedStocks.find(s => s.symbol === symbol);
          return {
            symbol: symbol,
                ...
                <Button 
                  onClick={() => {
                    if (selectedStocks.length > 0) {
                      ...
                      // setSelectedStocks(uploadedStocks.map(stock => stock.symbol)); // This line was commented out in the original file
                    }
                  }}
                  className="bg-blue-600 hover:bg-blue-700 shadow-md hover:shadow-lg transition-all duration-200"
                >
